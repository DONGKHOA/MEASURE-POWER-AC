 ...
 *****************************************************************************/
void
BSP_EXTI_Init (uint32_t pin, EXTI_EDGE_t edge)
{
  if (is_initialized == 0)
  {
    exti_event_queue = xQueueCreate(10, sizeof(uint32_t));
    xTaskCreate(BSP_EXTI_Task_Handler,
                "exti_handle_task",
                2048,
                NULL,
                10,
...
void
BSP_EXTI_SetCallbackFunction (EXTI_CALLBACK_FUNCTION_t CallbackFunction)
{
