...
esp_err_t
NVS_Erase_Flash (char *namespace)
{
  // Erase NVS flash
  nvs_handle_t nvsHandle;
  esp_err_t    retVal = nvs_open(namespace, NVS_READWRITE, &nvsHandle);
  if (retVal == ESP_OK)
  {
    nvs_erase_all(nvsHandle); // Xóa tất cả dữ liệu trong namespace cụ thể
    nvs_commit(nvsHandle); // Ghi lại thay đổi
    nvs_close(nvsHandle);  // Đóng handle
    printf("NVS namespace '%s' erased successfully.\n", namespace);
  }
  else
  {
...
NVS_WriteString (const char *name, const char *key, const char *stringVal)
{
  ...
  if (retVal != ESP_OK)
  {
    // ESP_LOGE("NVS", "Error (%s) opening NVS handle for Write",
    // esp_err_to_name(retVal));
  }
  else
  {
    ...
    if (retVal != ESP_OK)
    {
      // ESP_LOGE("NVS", "Error (%s) Can not commit - write",
      // esp_err_to_name(retVal));
    }
    else
    {
...
NVS_ReadString (const char *name, const char *key, char *savedData, uint8_t len)
{
  ...
  else
  {
    ...
    if (retVal == ESP_OK)
    {
      //    ESP_LOGW("NVS", "*****(%s) Can read/get value: %s",
    ...
    if (retVal != ESP_OK)
    {
      // ESP_LOGE("NVS", "Error (%s) Can not commit - read",
      // esp_err_to_name(retVal));
    }
    else
    {
