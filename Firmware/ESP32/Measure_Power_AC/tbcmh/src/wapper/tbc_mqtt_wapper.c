     ...
     //    mqtt_config->cert_pem = transport->verification.cert_pem;
        ...
        if (transport->credentials.type == TBC_TRANSPORT_CREDENTIALS_TYPE_X509) {
          ...
          //   mqtt_config->client_key_pem          = transport->authentication.client_key_pem;
            ...
            mqtt_config->credentials.authentication.key_password      = transport->authentication.client_key_password;
...
int tbcm_subscribe(tbcm_handle_t client, const char *topic, int qos /*=0*/)
{
     ...
     esp_mqtt_client_subscribe_single(client->mqtt_handle, topic, qos);
                            ...
                            int qos /*= 1*/, int retain /*= 0*/)
{
     ...
     if (client->config.log_rxtx_package) {
        // TBC_LOGI("[Attributes Request][Tx] request_id=%lu, %.*s",
                            ...
                            int qos /*= 1*/, int retain /*= 0*/)
{
     ...
     if (client->config.log_rxtx_package) {
        // TBC_LOGI("[Server-Side RPC][Tx] request_id=%lu Payload=%.*s",
                           ...
                           int qos /*= 1*/, int retain /*= 0*/)
{
     ...
     if (client->config.log_rxtx_package) {
        // TBC_LOGI("[Client-Side RPC][Tx] request_id=%lu %.*s",
          //     request_id, strlen(payload), payload);
     }
 ...
 */
                            ...
                            int qos /*= 1*/, int retain /*= 0*/)
 {
      ...
      if (client->config.log_rxtx_package)
      {
           // TBC_LOGI("[Provision][Tx] request_id=%lu %.*s",
                    // request_id, strlen(payload), payload);
      }
                          ...
                          int qos /*= 1*/, int retain /*= 0*/)
{
     ...
     if (client->config.log_rxtx_package) {
        // TBC_LOGI("[FW update][Tx] request_id=%lu chunk_id=%lu payload=%.*s",
                                      ...
                                      char *payload, int payload_len)
{
    ...
    if (strncmp(topic, TB_MQTT_TOPIC_ATTRIBUTES_RESPONSE_PREFIX,
         ...
         sscanf(temp, "%lu", &request_id);
         if (client->config.log_rxtx_package) {
             // TBC_LOGI("[Attributes Request][Rx] request_id=%lu %.*s",
         ...
         */
         ...
         if (chunk_str) {
             ...
             sscanf(temp, "%lu", &chunk_id);
         }

         if (client->config.log_rxtx_package) {
             // TBC_LOGI("[FW update][Rx] request_id=%lu, chunk_id=%lu, payload_len=%d",
